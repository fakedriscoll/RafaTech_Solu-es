            ...
            <form id="acessorio-form" class="space-y-2">
                ...
                <div>
                    ...
                    <select name="acessorio" class="w-full px-2 py-1 text-sm border border-gray-300 rounded" required>
                        <option value="">${translations[currentLang]['modal-form-servico-select'] || 'Selecione'}</option>
                        <option value="fone">${translations[currentLang]['modal-form-acessorio-fone'] || 'Fones de Ouvido'}</option>
                        <option value="capa">${translations[currentLang]['modal-form-acessorio-capa'] || 'Capas Protetoras'}</option>
...
if (contactForm) {
    contactForm.addEventListener("submit", function(e) {
        ...
        const currentUser = JSON.parse(localStorage.getItem("rafatech_current_user"));
        if (!currentUser || !currentUser.loggedIn) {
            alert("Por favor, faça login para enviar uma mensagem.");
            window.location.href = "login.html";
            return;
        }
        ...
        const mensagemTexto = document.getElementById("message").value;
        
        // Verificar se todos os campos obrigatórios foram preenchidos
        if (!nome || !email || !telefone || !assunto || !mensagemTexto) {
        ...
        let mensagem = `*Contato via Site RafaTech*%0A%0A`;
        mensagem += `*Nome:* ${nome}%0A`;
        mensagem += `*E-mail:* ${email}%0A`;
        mensagem += `*Telefone:* ${telefone}%0A`;
        mensagem += `*Assunto:* ${assunto}%0A`;
        mensagem += `*Mensagem:* ${mensagemTexto}%0A`;
...
document.querySelectorAll('.bg-blue-500.hover\\:bg-blue-600.text-white.text-xs.px-3.py-1.rounded-full').forEach(button => {
    button.addEventListener('click', function() {
        ...
        if (acessorioTitulo.includes('Fones') || acessorioTitulo.includes('Headphones')) {
            opcoes = [
                { nome: 'Fone Bluetooth TWS', preco: 'R$70,90' },
                { nome: 'Fone com Fio Estéreo', preco: 'R$20,00' },
                { nome: 'Headphone Over-ear', preco: 'R$150,90' },
                { nome: 'Fone Esportivo', preco: 'R$69,90' }
            ];
        } else if (acessorioTitulo.includes('Capas') || acessorioTitulo.includes('Cases')) {
            opcoes = [
                { nome: 'Capa Anti-impacto', preco: 'R$54,90' },
                { nome: 'Capa Transparente', preco: 'R$20,00' },
                { nome: 'Capa Carteira', preco: 'R$25,90' },
                { nome: 'Capa Personalizada', preco: 'R$45,00' }
            ];
        } else if (acessorioTitulo.includes('Carregadores') || acessorioTitulo.includes('Chargers')) {
            opcoes = [
                { nome: 'Carregador Rápido', preco: 'R$45,90' },
                { nome: 'Carregador Wireless', preco: 'R$54,90' },
                { nome: 'Carregador Veicular', preco: 'R$70,00' },
                { nome: 'Cabo USB Reforçado', preco: 'R$40,00' }
            ];
        } else if (acessorioTitulo.includes('Películas') || acessorioTitulo.includes('Protectors')) {
            opcoes = [
                { nome: 'Película de Vidro', preco: 'R$24,90' },
                { nome: 'Película Privacidade', preco: 'R$33,90' },
                { nome: 'Película Hidrogel', preco: 'R$34,90' },
                { nome: 'Película Câmera', preco: 'R$29,00' }
            ];
        }
...
// Configuração do Firebase para carregar produtos
...
// Função para carregar produtos dinâmicos do Firestore
async function loadDynamicProducts() {
    const loadingElement = document.getElementById("loading-dynamic-products");
    const gridElement = document.getElementById("dynamic-products-grid");
    const noProductsElement = document.getElementById("no-dynamic-products");
    ...
    try {
        ...
        gridElement.innerHTML = ""; // Limpar produtos existentes
        ...
        snapshot.forEach(doc => {
            ...
            const productCard = `
                <div class="bg-white rounded-lg overflow-hidden shadow-md hover:shadow-xl transition">
                    ...
                    <div class="p-4">
                        ...
                        <p class="text-green-600 text-xs mb-3">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <button onclick="consultarProduto('${product.name}', '${product.price.toFixed(2)}')" class="bg-blue-500 hover:bg-blue-600 text-white text-xs px-3 py-1 rounded-full">
                                Consultar
                            </button>
                        </div>
                    </div>
            ...
            `;
            gridElement.innerHTML += productCard;
        });
...
// Função para consultar produto específico via WhatsApp
function consultarProduto(nomeProduto, preco) {
    const currentUser = JSON.parse(localStorage.getItem("rafatech_current_user"));
    if (!currentUser || !currentUser.loggedIn) {
        alert("Por favor, faça login para consultar produtos.");
        window.location.href = "login.html";
        return;
    }
    
    const numeroWhatsApp = "5511998750637";
    let mensagem = `*Consulta de Produto - RafaTech*%0A%0A`;
    mensagem += `*Produto:* ${nomeProduto}%0A`;
    mensagem += `*Preço:* R$ ${preco}%0A%0A`;
    mensagem += `Olá! Gostaria de mais informações sobre este produto.`;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(loadDynamicProducts, 1000);
});
